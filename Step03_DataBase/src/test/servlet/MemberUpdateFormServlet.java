...
public class MemberUpdateFormServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. MemberDao 객체를 이용해서 해당 번호의 회원 정보를 얻어온다.
		...
		pw.println("<body>");
			...
			//회원정보 수정하려면 UPDATE member SET name=?, addr=? WHERE num=? 즉, name, addr, num 필요
			//그런데 num 은 고유번호니까 수정할 필요 없어. 그런데 위의 말처럼 UPDATE 에 필요는 하므로, 아예 안 써주지는 않고, hidden 으로 써 놓음
				//여기서 파라미터명 <=> name 명 ('num','name','addr')
